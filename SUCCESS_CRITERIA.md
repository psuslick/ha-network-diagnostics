# Network Diagnostics v0.3.0 success criteria

This file is the acceptance contract for the v0.3.0 package. It consolidates the decisions made through the prototype, privacy, architecture, and housekeeping reviews.
...
36. **Privacy applies to repository ownership metadata too.** The distributable ZIP uses repository-owner placeholders rather than embedding a personal account identity; publication finalization is an explicit separate step.
...
38. Python compilation, JavaScript syntax, repository validation, privacy validation, source-manifest validation, placeholder-finalization simulation, ZIP extraction validation, and checksum verification must pass before release.
...
40. **Validation must survive publication finalization.** The same automated test suite must pass both in the privacy-preserving distributable state (repository-owner placeholders present) and after `finalize_repo.py` injects a valid publisher/repository identity. Repository metadata finalization must not change product behavior or invalidate tests.
